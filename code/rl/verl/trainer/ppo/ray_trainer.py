    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Log per-component reward means (answer_reward, format_reward, etc.)
                if reward_extra_infos_dict:
                    for _key, _vals in reward_extra_infos_dict.items():
                        try:
                            _arr = np.asarray(_vals, dtype=float)
                            if _arr.ndim == 1 and len(_arr) > 0:
                                metrics[f"train_reward/{_key}/mean"] = float(_arr.mean())
