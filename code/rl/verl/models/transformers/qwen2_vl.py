...
def qwen2_vl_attn_forward(
    ...
    **kwargs,
    ...
    # Compatibility:
    # - older/newer checkpoints may store mrope metadata in different fields.
    # - this patch previously assumed self.rope_scaling always exists.
    rope_meta = getattr(self, "rope_scaling", None)
    if rope_meta is None:
        rope_meta = getattr(getattr(self, "config", None), "rope_scaling", None)
    if rope_meta is None:
        rope_meta = getattr(getattr(self, "config", None), "rope_parameters", None)
    if rope_meta is None or "mrope_section" not in rope_meta:
        raise ValueError(
            "Missing mrope_section in rope metadata. "
            f"rope_scaling={getattr(self, 'rope_scaling', None)}, "
            f"config.rope_scaling={getattr(getattr(self, 'config', None), 'rope_scaling', None)}, "
            f"config.rope_parameters={getattr(getattr(self, 'config', None), 'rope_parameters', None)}"
        )
    query_states, key_states = apply_multimodal_rotary_pos_emb(
        query_states, key_states, cos, sin, rope_meta["mrope_section"]
    )
