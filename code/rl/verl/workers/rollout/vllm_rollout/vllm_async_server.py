...
class vLLMHttpServer:
    ...
    async def generate(
        ...
        logprobs_param = sampling_params.pop("logprobs", None)
        if logprobs_param is True:
            sampling_params["logprobs"] = 0  # record only the generated token's logp
        elif isinstance(logprobs_param, int) and logprobs_param > 0:
            sampling_params["logprobs"] = logprobs_param  # top-K (used for score calls)
        else:
    ...
    async def score_token_logprob(
        ...
        """Score a prefix and return the top-K token logprobs for the next token.

        Uses max_tokens=1 and temperature=0 so this is a prefill-only call.
        Returns a dict mapping token_id -> logprob, or None on error.
        """
        try:
            max_possible_tokens = self.config.max_model_len - len(prompt_ids)
            if max_possible_tokens <= 0:
            ...
            prompt_ids_deduped = _qwen2_5_vl_dedup_image_tokens(prompt_ids, self.model_config.processor)
            multi_modal_data = {}
            if image_data is not None:
                multi_modal_data["image"] = image_data
            prompt = TokensPrompt(prompt_token_ids=prompt_ids_deduped, multi_modal_data=multi_modal_data)
            ...
            token_logprob_map = {int(tid): lp_obj.logprob for tid, lp_obj in logprobs_list[0].items()}
            return token_logprob_map
        except Exception:
            logger.exception("score_token_logprob failed for request_id=%s", request_id)
            return None
